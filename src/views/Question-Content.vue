    ...
    <el-card>
        ...
        <div class="flex-start user-info">
            ...
            <div class="question-info">
                <div class="user-name">
                    <router-link :to="{ name: 'User-Homepage', params: { username: this.question.author.username }}">
                        {{ this.question.author.fullName }}
                    </router-link>
                </div>
...
export default {
    ...
    data() {
        return {
            question: {author: {username:"root",fullName:""}},
            pageNum: null,
            answers: {author: {username:"root",fullName:""}},
            size: 0,
            content: null,
            loading: false
        }
    ...
    methods: {
        getQuestion() {
            getQuestion(this.$route.params.id).then(result => {
                ...
                console.log("created")
                console.log(this.question.author.username)
            })
        ...
        goBack() {
            // this.$router.back()
            console.log("刷新"+this.question.courseId)
            // location.reload()
            // this.$router.go(-1)(this.to === undefined)?this.$router.go(-1):
            // this.$router.push({path:this.to})
            
            this.$router.push({ name: 'Course-Content', params: { id: this.question.courseId }})
