<template>
    <el-form :model="chapter" :rules="rules" ref="chapter" label-width="60px">
        ...
        <el-form-item  prop="textContent" label="内容">
            <el-input type="textarea" v-model="chapter.textContent" :autosize="{minRows: 10}"
                      maxlength="500" show-word-limit/>
        </el-form-item>
    ...
    methods: {
        ...
        onSubmit(ref) {
            if (this.editMode === 'create') {
                // print(this.$route.query.sectionId)
                this.$refs[ref].validate((valid) => {
                    if (valid) {
                        ...
                        this.chapter.sectionId = this.sectionId
            ...
            console.log(this.chapter.sectionId)
            console.log(this.$route.query.sectionId)
            let id = this.chapter.sectionId!=undefined?this.chapter.sectionId:this.$route.query.sectionId
            console.log("id"+id)
            if(redirect.startsWith("/teaching")){
                this.$router.push({name: 'Teaching-Chapter-List',query: {sectionId: id}})
                // this.$router.go(0)
            }else{
