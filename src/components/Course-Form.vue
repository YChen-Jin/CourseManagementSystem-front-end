    ...
    export default {
        ...
        methods: {
            ...
            onSubmit(ref) {
                ...
                // this.$router.go(-1) // this.$router.go({path: redirect})
                let redirect = this.$route.fullPath
                console.log(this.$route.fullPath)

                console.log(redirect.includes("update"))
                if(redirect.startsWith("/user")){
                    this.$router.push({name: 'Index'})
                }else if(redirect.startsWith("/teaching")){
                    console.log(redirect)
                    console.log(this.courseId)
                    this.$router.push({name: 'Teaching-Course-List'})
                    // if (redirect.includes("update")){
                    //     this.$router.push({name: 'Teaching-Course-List'})
                    // } else if (redirect.includes("create")){
                    //     // this.$router.push({name: 'Teaching-Section-List', query: {courseId: course.id}})

                    //     this.$router.push({name: 'Teaching-Course-Create',query:{step:2,courseId: this.course.id}})
                    // }
                    // this.$router.go(0)
                } else {
